...
const cardsLastDelay = getDelay(features.length - 1) + 0.5;
const buttonDelay = cardsLastDelay - 0.45;
  ...
  <section className="min-h-screen w-full py-36 page-bg relative overflow-x-hidden">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12">
          {features.map((f, idx) => (
            <Card
              key={f.title}
              className="group p-8 relative bg-white/80 border border-border/30 shadow-xl hover:scale-105 hover:shadow-2xl hover:border-primary/40 transition-all duration-300 animate-snake-in"
      ...
      <style>{`
        ...
        .animate-fade-up {
          ...
          animation: fadeUp 1s cubic-bezier(.39,1.69,.36,1) both;
        }
        @keyframes fadeUp {
          to { opacity: 1; transform: translateY(0);}
        }
